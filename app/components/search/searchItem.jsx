...
export default function SearchItem({ flightInfo, onClick, isSelected }) {
  ...
  return (
    <>
      <div
        className={`px-4 flex mb-4 text-grey-900 cursor-pointer rounded-lg ${
          isSelected ? "border rounded-lg border-purpleBlue bg-purpleWhite" : ""
        }`}
        onClick={onClick}
      >
        <Image
        ...
        <div className="flex   flex-auto p-4 ">
          ...
          <div className="flex flex-1 flex-col items-end">
            ...
            <p className="text-grey-400">
              {flightInfo.stops != 0 ? `${flightInfo.stopLength}` : ""}
            </p>
          </div>
          <div className="flex flex-1 flex-col items-end">
